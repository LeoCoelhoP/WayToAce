...
function saveTimeLeftOnLocalStorage(examId, remainingTime) {
	try {
		const timers = JSON.parse(localStorage.getItem('timers'));
		let newTimers;
		if (timers && timers.length > 0) {
			newTimers = timers.map((el) =>
				el?.examId === examId ? { ...el, remainingTime } : el,
			);
		} else {
			newTimers = [{ examId, remainingTime }];
		}
...
export default function Timer({
	...
	function handleNavigateBack() {
		if (window.confirm(t('closeExamConfirm'))) {
			saveTimeLeftOnLocalStorage(examId, totalSeconds);
			navigate('/');
		}
	}
	...
	return (
		<div className='z-30 flex flex-col items-center justify-center w-full gap-2 p-2 shadow-lg dark:bg-zinc-800 dark:text-zinc-200'>
			<span
